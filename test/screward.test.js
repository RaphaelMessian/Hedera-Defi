...
const fileId = FileId.fromString("0.0.48903942");
...
describe('All Tests', async function () {
    ...
    let aliceKey;
    let bobAccountId;
    let createStackingToken;
    let createRewardToken1;
    let createRewardToken2;
    let contractId;
    before(async function () {
        aliceKey = PrivateKey.generateED25519();
        aliceAccountId = await createAccount(client, aliceKey, 10);
        // const BobKey = PrivateKey.generateED25519();
        // bobAccountId = await createAccount(client, BobKey, 10);
        createStackingToken = await createFungibleToken("Stacking Token", "ST", aliceAccountId, aliceKey.publicKey, client, aliceKey);
